...
class vrpnVelCalc:
    ...
    def __init__(self):
        # Define subscribers
        self.pose_sub = rospy.Subscriber('/vrpn_client_node/RadarQuad/pose', PoseStamped, self.pose_cb, queue_size=10)
        # Define publishers
        ...
        self.body_vel_pub = rospy.Publisher('/vrpn_velocity/vehicle_1_frame/filtered', TwistStamped, queue_size=10)
...
#        print(time_diff_s)
        if (time_diff_s > .005):
            ...
            self.body_vel_msg.twist.linear.x = -1*(math.cos(current_yaw)*self.filt_vel_msg.twist.linear.x + math.sin(current_yaw)*self.filt_vel_msg.twist.linear.y)
